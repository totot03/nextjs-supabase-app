...
- Next.js **16.3.0**, React 19.2.8, TypeScript strict, ESLint flat config (`next/core-web-vitals` + `next/typescript`)
...
npm run lint     # eslint . (유일한 정적 검사 수단, 별도 test 스크립트 없음)
```

CI(`.github/workflows`)가 없으므로 `npm run build`와 `npm run lint`는 커밋 전에 직접 실행해서 확인한다.
